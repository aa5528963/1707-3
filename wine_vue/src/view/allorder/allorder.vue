			...
			<div class="allOrder">
				<ul v-for="(itm,index) in orderList">
					...
					<li v-for="(val,key) in itm">
						<img :src="val.imgurl" alt="" class="fl">
						<!-- {{val.id}} -->
			...
			deleteOrder:function(id,e){
				...
				axios.get(http.url+'/orderdel',{
					...
					axios.get(
		                ...
		                //console.log(res.data.data.results[1].goods)
		...
		beforeMount:function(){
           axios.get(
                ...
                for(var j=0;j<goodsList.length;j++){
                    ...
                    goodsList[j].orderId = _res[j].id;
                    goodsList[j].orderTotal = _res[j].total;
                    goodsList[j].orderType = _res[j].type;
                    console.log(goodsList[j]);
                }
                this.orderList = goodsList;
            })
		...
		main{
			...
			.allOrder{
				...
				ul{
					...
					p{
						font-size:0.4rem;color:#f00;line-height:0.8rem;
					...
					li{
						...
						.price{font-size:0.35rem;line-height:2rem;color:#f00;}
						.price::before{content:"￥"}
						span{font-size:0.35rem;line-height:0.6rem;}
					}
				}
